...
#. (Optional) If the rule follows a schedule, you can set a :guilabel:`Recurrence` period for the muting rule. A scheduled muting rule can repeat on a daily or weekly basis. Select :menuselection:`Custom` to set a custom recurrence period.
