...
.. note:: If you want a custom event to show up in the UI, you must set the attribute ``workflow.name`` for it. Without the ``workflow.name`` attribute, the span is still recorded but will only show up in the request waterfall view.
