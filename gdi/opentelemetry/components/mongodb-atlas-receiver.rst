...
.. list-table::
   ...
   - 
      ...
      - DB operation rates aggregate of MongoDB metrics
         ``OPCOUNTER_GETMORE``, ``OPERATIONS_SCAN_AND_ORDER``,
         ``OPCOUNTER_UPDATE``, ``OPCOUNTER_REPL_UPDATE``,
         ``OPCOUNTER_CMD``, ``OPCOUNTER_DELETE``,
         ``OPCOUNTER_REPL_DELETE``, ``OPCOUNTER_REPL_CMD``,
         ``OPCOUNTER_QUERY``, ``OPCOUNTER_REPL_INSERT``,
         ``OPCOUNTER_INSERT``
      - ``{operations}/s``
      - Gauge (Double)
      - ``operation cluster_role``
   - 

      - mongodbatlas.process.db.operations.time
      - Default
      - DB operation times aggregate of MongoDB metrics
         ``OP_EXECUTION_TIME_WRITES``, ``OP_EXECUTION_TIME_COMMANDS``,
   ...
   - 
      ...
      - Scanned objects aggregate of MongoDB metrics
         ``QUERY_EXECUTOR_SCANNED_OBJECTS``, ``QUERY_EXECUTOR_SCANNED``
      - ``{objects}/s``
      - Gauge (Double)
      - ``scanned_type``
   - 
   ...
   - 
      ...
      - Storage used by the database aggregate of MongoDB metrics
         ``DB_INDEX_SIZE_TOTAL``, ``DB_DATA_SIZE_TOTAL_WO_SYSTEM``,
   ...
   - 
      ...
      - Default
      - Journals written MongoDB metric ``JOURNALING_MB``
      - ``MiBy``
      - Gauge (Double)
      - 
   - 
      ...
      - Memory usage aggregate of MongoDB metrics
         ``MEMORY_MAPPED, MEMORY_VIRTUAL``, ``COMPUTED_MEMORY``,
         ``MEMORY_RESIDENT``
      - ``By``
      - Gauge (Double)
      - ``memory_state``
   - 
      ...
      - Default
      - Network IO aggregate of MongoDB metrics ``NETWORK_BYTES_OUT``,
         ``NETWORK_BYTES_IN``
      - ``By/s``
      - Gauge (Double)
      - ``direction``
   - 
      ...
      - Default
      - Network requests MongoDB metric ``NETWORK_NUM_REQUESTS``
      - ``{requests}``
      - Sum(Double)
      - 
   - 
   ...
   - 
      ...
      - Execution time by operation aggregate of MongoDB metrics
         ``OPLOG_MASTER_TIME``, ``OPLOG_SLAVE_LAG_MASTER_TIME``,
   ...
   - 
      ...
      - Page faults aggregate of MongoDB metrics
         ...
         ``EXTRA_INFO_PAGE_FAULTS``, ``GLOBAL_ACCESSES_NOT_IN_MEMORY``
      - ``{faults}/s``
      - Gauge (Double)
      - ``memory_issue_type``
   - 
   ...
   - 
      ...
      - Tickets aggregate of MongoDB metrics
         ``TICKETS_AVAILABLE_WRITE``, ``TICKETS_AVAILABLE_READS``
      - ``{tickets}``
      - Gauge (Double)
      - ``ticket_type``
   - 
      ...
      - System CPU normalized to pct aggregate of MongoDB metrics
         ...
         ``SYSTEM_NORMALIZED_CPU_GUEST``, ``SYSTEM_NORMALIZED_CPU_IRQ``,
         ``SYSTEM_NORMALIZED_CPU_KERNEL``,
         ``SYSTEM_NORMALIZED_CPU_STEAL``,
         ``SYSTEM_NORMALIZED_CPU_SOFTIRQ``,
         ``SYSTEM_NORMALIZED_CPU_NICE,`` ``SYSTEM_NORMALIZED_CPU_USER``
      - ``1``
      - Gauge (Double)
      - ``cpu_state``
   - 
   ...
   - 
      ...
      - System CPU usage (%) aggregate of MongoDB metrics
         ``SYSTEM_CPU_USER``, ``SYSTEM_CPU_GUEST``,
         ``SYSTEM_CPU_SOFTIRQ``, ``SYSTEM_CPU_IRQ``,
         ``SYSTEM_CPU_KERNEL``, ``SYSTEM_CPU_IOWAIT``,
         ``SYSTEM_CPU_NICE``, ``SYSTEM_CPU_STEAL``
      - ``1``
      - Gauge (Double)
      - ``cpu_state``
   - 
      ...
      - Default
      - System CPU usage (%) aggregate of MongoDB metrics
         ``MAX_SYSTEM_CPU_SOFTIRQ``, ``MAX_SYSTEM_CPU_IRQ``,
         ``MAX_SYSTEM_CPU_GUEST``, ``MAX_SYSTEM_CPU_IOWAIT``,
         ``MAX_SYSTEM_CPU_NICE``, ``MAX_SYSTEM_CPU_KERNEL``,
         ``MAX_SYSTEM_CPU_USER``, ``MAX_SYSTEM_CPU_STEAL``
      - ``1``
      - Gauge (Double)
      - ``cpu_state``
   - 
   ...
   - 
      ...
      - Default
      - Full text search disk usage MongoDB metric ``FTS_DISK_USAGE``
      - ``By``
      - Gauge (Double)
      - 
   - 
      ...
      - Full-text search aggregate of MongoDB metrics
         ``FTS_MEMORY_MAPPED``, ``FTS_PROCESS_SHARED_MEMORY``,
   ...
   - 
      ...
      - System memory usage aggregate of MongoDB metrics
         ``SYSTEM_MEMORY_AVAILABLE``, ``SYSTEM_MEMORY_BUFFERS``,
         ``SYSTEM_MEMORY_USED``, ``SYSTEM_MEMORY_CACHED``,
         ``SYSTEM_MEMORY_SHARED``, ``SYSTEM_MEMORY_FREE``
      - ``KiBy``
      - Gauge (Double)
      - ``memory_state``
   - 
      ...
      - Default
      - System memory usage aggregate of MongoDB metrics
         ``MAX_SYSTEM_MEMORY_CACHED``, ``MAX_SYSTEM_MEMORY_AVAILABLE``,
         ``MAX_SYSTEM_MEMORY_USED``, ``MAX_SYSTEM_MEMORY_BUFFERS``,
         ``MAX_SYSTEM_MEMORY_FREE``, ``MAX_SYSTEM_MEMORY_SHARED``
      - ``KiBy``
      - Gauge (Double)
      - ``memory_state``
   - 
   ...
   - 
      ...
      - Default
      - System Network IO aggregate of MongoDB metrics
         ``MAX_SYSTEM_NETWORK_OUT``, ``MAX_SYSTEM_NETWORK_IN``
      - ``By/s``
      - Gauge (Double)
      - ``direction``
   - 
      ...
      - Default
      - Swap IO aggregate of MongoDB metrics ``SWAP_IO_IN, SWAP_IO_OUT``
      - ``{pages}/s``
      - Gauge (Double)
      - ``direction``
   - 
   ...
   - 
      ...
      - Swap usage aggregate of MongoDB metrics
         ``SWAP_USAGE_FREE, SWAP_USAGE_USED``
      - ``KiBy``
      - Gauge (Double)
      - ``direction``
   - 
      ...
      - Default
      - Swap usage aggregate of MongoDB metrics ``MAX_SWAP_USAGE_FREE``,
         ``MAX_SWAP_USAGE_USED``
      - ``KiBy``
      - Gauge (Double)
      - ``direction``
