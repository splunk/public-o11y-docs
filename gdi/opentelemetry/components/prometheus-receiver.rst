...
The Prometheus Receiver can map Prometheus metrics to OpenTelemetry proto-based metrics. The Prometheus Receiver maintains the original metric name, value, timestamp, as well as tags. 
...
The Prometheus Receiver is meant to be a drop-in replacement for Prometheus to scrape your services. However, there are advanced features of Prometheus that are not supported, and do return an error if the receiver configuration contains any of the following options:
