...
The rules for associating the data passing through the processor with specific Pod metadata are configured using the ``pod_association`` field, which represents a list of associations that are executed in the specified order until the first one is able to match.

Each association is specified as a list of sources of associations. Sources represent list of rules. All rules are executed, resulting in a metadata cache key. In order to apply an association, all the data defined in each source has to be successfully retrieved from a log, trace or metric.
...
Container spec attributes - will be set only if container identifying attribute k8s.container.name is set as a resource attribute (similar to all other attributes, pod has to be identified as well):
container.image.name
container.image.tag
Container attributes - in addition to pod identifier and k8s.container.name attribute, k8s.container.restart_count resource attribute is needed to get association with a particular container instance, but not required. If k8s.container.restart_count is not set, the latest container instance will be used:
