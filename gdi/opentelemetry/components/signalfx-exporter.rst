...
By default, the Splunk Distribution of OpenTelemetry Collector includes the SignalFx exporter in the ``traces``, ``metrics``, and ``logs/signalfx`` pipelines when deploying in Host monitoring mode. See :ref:`otel-deployment-mode` for more information.
