...
The SignalFx exporter allows the OpenTelemetry Collector to send traces, logs, and metrics to SignalFx endpoints. The supported pipeline types are ``traces``, ``metrics``, and ``logs``. See :ref:`otel-data-processing` for more information.
...
By default, the Splunk Distribution of OpenTelemetry Collector includes the SignalFx exporter in the ``logs`` pipeline when deploying in agent mode. See :ref:`otel-deployment-mode` for more information.

.. note:: To send SignalFx data through a proxy, configure proxy settings as environment variables. See :ref:`configure-proxy-collector` for more information.

Sample configurations
----------------------

The following example shows a SignalFx exporter instance configured for a logs pipeline in the Collector configuration file:
...
The SignalFx exporter requires a SignalFx token and endpoint. Obtaining a HEC token and choosing a HEC endpoint depends on the target. The following table shows endpoints and instructions for each back end.
