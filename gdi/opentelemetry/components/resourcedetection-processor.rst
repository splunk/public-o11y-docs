...
By default, the Splunk Distribution of OpenTelemetry Collector includes the resource detection processor in all the predefined pipelines when deploying in Host monitoring mode. When deploying the Collector in Data forwarding mode, the resource detection processor collects internal metrics. See :ref:`otel-deployment-mode` for more information.
