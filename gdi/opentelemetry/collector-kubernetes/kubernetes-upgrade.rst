...
#. Deploy the new access token with Helm upgrade. If you want to use the latest version, remove ``'--version <Current Chart Version>'``: 

.. code-block:: bash

  helm upgrade --reuse-values --version <Current Chart Version> --set splunkObservability.accessToken=<New Access Token> <Release Name> splunk-otel-collector-chart/splunk-otel-collector 
...
.. code-block:: bash
  ...
  kubectl rollout restart daemonset <Release Name>-agent kubectl rollout restart deployment <Release Name>-k8s-cluster-receiver # Optional, if the gateway is enabled kubectl rollout restart deployment <Release Name> 

#. Check the status of your clusters' pods: 

.. code-block:: bash

  kubectl get pod -n <Namespace> | grep <Release Name>