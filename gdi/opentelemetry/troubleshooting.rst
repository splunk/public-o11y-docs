.. _otel-troubleshooting:

*************************************************************************
Troubleshoot issues when collecting data
*************************************************************************

.. meta::
      :description: Describes known issues when collecting telemetry data using the OpenTelemetry Collector and the Splunk Distribution of OpenTelemetry Collector.

.. toctree::
    :maxdepth: 4
    :titlesonly:
    :hidden:

    splunk-collector-troubleshooting.rst
    troubleshoot-k8s.rst
    support-checklist.rst
    open-support-request.rst

Here are some common issues that you might encounter when using OpenTelemetry Collector and the Splunk Distribution of OpenTelemetry Collector, and tips on how to address them. 

* :ref:`otel-splunk-collector-tshoot`
* :ref:`tshoot-k8s-container-runtimes`
* :ref:`otel-support-checklist`
* :ref:`support`
