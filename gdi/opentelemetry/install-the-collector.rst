...
* Splunk Distribution of OpenTelemetry Collector for Kubernetes or ``splunk-otel-collector-chart``. See :ref:`Install on Kubernetes <otel-install-k8s>`. You can also install the Collector and Operator for Auto Instrumentation, as explained in :ref:`Install on Kubernetes with the Operator <auto-instrumentation-operator>`.
