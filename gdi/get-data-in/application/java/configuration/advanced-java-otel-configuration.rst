...
.. list-table:: 
   ...
   * - ``SPLUNK_TRACE_RESPONSE_HEADER_ENABLED``
     - Activates the addition of server trace information to HTTP response headers. For more information, see :ref:`server-trace-information-java`. The default value is ``true``. |br| |br| System property: ``splunk.trace-response-header.activated``
...
.. list-table:: 
   ...
   * - ``SPLUNK_PROFILER_ENABLED``
     - Activates AlwaysOn Profiling. The default value is ``false``. |br| |br| System property: ``splunk.profiler.activated``
   ...
   * - ``SPLUNK_PROFILER_MEMORY_ENABLED``
     - Activates memory profiling with all the options. Enabling memory profiling overrides the value of ``splunk.metrics.activated``. The default value is ``false``. Requires ``splunk.profiler.activated`` to be set to ``true``. To activate or deactivate specific memory profiling options, set their values explicitly. |br| |br| System property: ``splunk.profiler.memory.activated``
   ...
   * - ``SPLUNK_PROFILER_TLAB_ENABLED``
     - Whether to activate TLAB memory events. The default value is the value assigned to the ``splunk.profiler.memory.activated`` property. |br| |br| System property: ``splunk.profiler.tlab.activated``
...
.. list-table:: 
   ...
   * - ``SPLUNK_METRICS_ENABLED``
     - Activates exporting metrics. If you activate memory profiling using the ``splunk.profiler.memory.activated`` property, the value of ``splunk.metrics.activated`` is ignored. See :ref:`java-otel-metrics-attributes` for more information. Default is ``false``. |br| |br| System property: ``splunk.metrics.activated``
...
.. list-table:: 
   ...
   * - ``OTEL_JAVAAGENT_ENABLED``
     - Globally activates the Java agent automatic instrumentation. The default value is ``true``. Useful for deactivating auto instrumentation in testing scenarios or pipelines. |br| |br| System property: ``otel.javaagent.activated`` 
