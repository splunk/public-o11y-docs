...
Decide which CloudFormation template to use depending on your deployment method (for example, per AWS region or per AWS account) and integration type (for example, metric streams only). Even if you don't intend to use all options, you can safely deploy a CloudFormation template, since unused infrastructure doesn't generate costs.
...
Select the QuickLink for a template which supports Metric Streams, it will open the AWS Management Console in the last region that you used.
