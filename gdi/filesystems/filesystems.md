...
On Linux hosts, this monitor relies on the `/proc` filesystem. If the underlying hos `/proc` file system is mounted somewhere other than `/proc`, specify the path using the top level configuration `procPath`, as shown in the following example:
