...
The [Splunk Distribution of OpenTelemetry Collector](https://github.com/signalfx/splunk-otel-collector) provides this integration as the `expvar` monitor via the [Smart Agent Receiver](https://github.com/signalfx/splunk-otel-collector/tree/main/internal/receiver/smartagentreceiver).
...
This is an example showing the minimal required expvar monitor configuration for exporting
...
- type: expvar
  ...
  port: 8000
```
Include the extra dimension `metric_source` with a meaningful value in order to facilitate
...
```
This is an example showing part of a JSON payload containing the exposed variable `requestsPerSecond` containing
...
configuration configures the monitor to fetch all 2 `Mallocs` values (35387 and 35387). Two data points
...
The monitor sends the following metrics to Splunk Observability Cloud. This monitor will also emit by default any metrics that are not listed in the following table:

<div class="metrics-table" type="expvar" include="markdown"></div>
