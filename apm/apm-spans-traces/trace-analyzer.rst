...
Within the Trace Analyzer interface there are a 3 different max trace limits. 

1. The Trace Analyzer chart and the Group Metrics (2) tab display the full count of traces that match the time window you select. However, the list of traces displayed on the Traces tab (3) has a limit of 1,000 traces. Trace Analyzer searches for traces starting at the end of the time window you select. When Trace Analyzer has matched 1,000 traces, the list stops populating. 
