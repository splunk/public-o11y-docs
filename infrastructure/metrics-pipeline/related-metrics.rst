...
.. list-table::
   ...
   * - ``sf.org.numDatapointsDroppedThrottle``
     - Number of data points you sent to Infrastructure Monitoring that Infrastructure Monitoring didn't accept because your organization significantly exceeded its data points per minute (DPM) limit. For help with this issue, reach out to support at observability-support@splunk.com. Unlike ``sf.org.numDatapointsDroppedExceededQuota``, this metric represents data points for both existing and new metric time series (MTS). If Infrastructure Monitoring is throttling your organization, it isn't keeping any of your data.
