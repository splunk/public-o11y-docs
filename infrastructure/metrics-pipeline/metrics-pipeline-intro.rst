...
For example, suppose your systems generate data points every five seconds. Two successive data points have timestamps
that differ by five seconds. If your systems immediately transmit the points to Observability Cloud, the system ingests
two data points every ten seconds. Metrics pipeline management can roll up the two data points into one aggregated
...
Observability Cloud might receive two data points in the first ten seconds, then twelve data points in the next ten
...
The difference between the timestamp your systems add to a raw data point when it's created and the time
we use when we aggregate data points can cause one of the following issues:
...
* The starting and ending time of aggregated MTS might shift. A data point generated by your server
  ...
  MTS shifts right on the chart, indicating that the start time was later than the actual timestamp. This shift occurs
  because metrics pipeline management ignores the data point timestamp and instead uses the time it ingest the
  data point.
...
* The duration of aggregated MTS might be distorted.

Solutions
--------------------------------------------------------------------------------

Avoid avoid these aggregation issues by using the following options:

* Do your own MTS aggregation before sending data to us, by reconfiguring the OTel collector to drop unwanted dimensions.
