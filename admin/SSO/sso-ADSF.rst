...
#. Configure this rule to pass through Name ID, if it's not already provided by your ADFS or SAML implementation.
   ...
   #. Regardless of the types you select, from the :guilabel:`Outgoing name ID format` list select :guilabel:`Persistent Identifier`.
...
#. Select :guilabel:`Detail`, then select :guilabel:`Copy to file`. The certificate export guided setup appears.
